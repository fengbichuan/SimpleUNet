...
#     """
#
#     def __init__(self, stage_channels, num_blocks, short_rate, ks, pad, dilation, adw):
#         super().__init__()
...
#             # 解码器卷积块
#             de_layers_block = []
#             de_layers_block.append(SingleConv(int(short_rate * (re_stage_channels[i - 1] + re_stage_channels[i])),
#                                               int(short_rate * re_stage_channels[i]),
#                                               ks, pad, dilation))
#             for _ in range(re_num_blocks[i] - 1):
#                 de_layers_block.append(
#                     SingleConv(int(short_rate * re_stage_channels[i]), int(short_rate * re_stage_channels[i]),
#                                ks, pad, dilation))
#             self.de_layers.append(nn.Sequential(*de_layers_block))
#
#             # 2. 注意力权重 (Alpha 和 Beta)
#             alpha = nn.Parameter(torch.ones(int(short_rate * re_stage_channels[i]), 1, 1), requires_grad=adw)
#             beta = nn.Parameter(torch.ones(int(short_rate * re_stage_channels[i - 1]), 1, 1), requires_grad=adw)
...
#             # 融合: [跳跃连接 * alpha] 和 [上采样特征 * beta]
#             y = torch.concat([re_shortcuts[j] * self.alphas[j],
#                               self.up(x * self.betas[j])], dim=1)
#             # 通过解码器卷积块
...
#
# # ----------------------------------------------------------------------
# # 6. 重构后的 SimpleUNet (主模块) (修改: 增加了Bottleneck)
# # ----------------------------------------------------------------------
...
#
#         # 4. 实例化解码器
#         self.decoder = Decoder(stage_channels, num_blocks, short_rate, ks, self.pad, dilation, adw)
#
#         # 5. 实例化最终的分割头 (1x1 卷积)
...
#         model = SimpleUNet(in_channels=3, num_cls=1, stage_channels=[16, 32, 64, 128, 256], num_blocks=[1, 1, 1, 1, 1],
#                            short_rate=0.5, adw=True).cuda()
#
...
class Decoder(nn.Module):
    ...
    def __init__(self, stage_channels, num_blocks, short_rate, ks, pad, dilation):
        ...
        # 1. 解码器层 (de_layer0, de_layer1, ...)
        self.de_layers = nn.ModuleList()
        # self.alphas = nn.ParameterList()  <-- 已移除
        # self.betas = nn.ParameterList()   <-- 已移除
